...
date_sta="17:40"# thoi gian gui bao cao hoc sinh chua uong
date_str="23:14"# thoi gian gui bao cao tong su dung vaf gui cho phu huynh
...
def xulyQR():
    global cur, dangxuly, student, student_name, day, weekdays, gui_canhbao,gui_baocao
    ...
    while cap.isOpened():
        ...
        if len(corners) > 0 and dangxuly == 0:
            ...
            for (markerCorner, markerID) in zip(corners, ids):
                ...
                try:
                    #dt.strftime('%A')
                    cur.execute(f"SELECT {weekdays[1]}  FROM weekreport WHERE ID=%a" % student)
                    # tìm nạp các hàng từ đối tượng con trỏ
                    result = cur.fetchall()
                    data = numpy.array(result)
                    if data[0][0] != '':
...
class UI(QtWidgets.QMainWindow, main_page.Ui_MainWindow):
    ...
    def ok(self):
        global student,student_name,drink,type,types,myconn,dangxuly
        dt = datetime.now()
