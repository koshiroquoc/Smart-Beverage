...
date_sta="16:30"# thoi gian gui bao cao hoc sinh chua uong
date_str="16:31"# thoi gian gui bao cao tong su dung va gui cho phu huynh
    ...
    while True:
        ...
        if dt.isoweekday() < 6:  # kiem tra ngay trong tuan
            ...
            if dt.strftime("%H:%M") == date_sta:  # ddung thoi gian
                ...
                gui_canhbao = 0
            if dt.strftime("%H:%M") == date_str and dt.strftime('%A') == 'Thursday':
                if gui_baocao == 0:
                    print("gui bao cao")
                    cur = myconn.cursor()
                    try:
...
def xulyQR():
    global cap,cur, dangxuly, student, student_name, day, weekdays, gui_canhbao,gui_baocao,dt,vichat,luongnuoc,suckhoe
        ...
        if len(corners) > 0 and dangxuly == 0:
            ...
            for (markerCorner, markerID) in zip(corners, ids):
                ...
                if dauong==0:
                    ...
                    cur.execute(sql)           #truy van id cua hs
                    myresult = cur.fetchall()
                    student_name = numpy.array(myresult)[0][0]
                    # laasy thong tin vi chat
                    sql = "SELECT STATUS FROM users WHERE ID = %d" % (student)
                    cur.execute(sql)            #truy van vi chat
                    myresult = cur.fetchall()
                    vichats = numpy.array(myresult)
                    if vichats[0][0] == '' or (vichats[0][0] is None):
                        print("Khong vi chat")
                        vichat = 0
                    else:
                        print("vichat")
                    ...
                    #luu luong
                    sql = "SELECT VAR FROM users WHERE ID = %d" % (student)
                    cur.execute(sql)
                    myresult = cur.fetchall()
                    luongnuoc = numpy.array(myresult)[0][0]
                    print("luong nuoc")
                    print(luongnuoc)
                    #########
                    mixer.init()                 #khoi tao mixer de phat nhac
                    mixer.music.load(str('./audio' + '/{}.mp3'.format(str(student))))
                    mixer.music.play()
                    while mixer.music.get_busy():  # wait for music to finish playing
                        time.sleep(0.1)
                    # Phát tập tin âm thanh
                    #playsound(str('./audio' + '/{}.mp3'.format(str(student))))
...
class UI(QtWidgets.QMainWindow, main_page.Ui_MainWindow):
    ...
    def suatuoi(self):
        global student_name,drink,drinks,vichat,suckhoe
        if dangxuly==1 and (vichat!=2 and vichat!=1):
            ...
            if suckhoe==4:
                self.uic2.scroll_custem.setDisabled(True)
                self.uic2.radio_id.setDisabled(True)
                self.uic2.radio_nd.setDisabled(True)
            drink=drinks[0]
            print("khơi tạo ok")
            print("suatuoi")
        elif (vichat==1 or vichat==2):
            QMessageBox.about(self, "Title", "Bạn đang uống vi chất nên chỉ uống được nước cam thôi nhé")
    def suadau(self):
        global student_name,drink,vichat,suckhoe
        if dangxuly == 1 and (vichat!=2 and vichat!=1):
            self.suatuoi = QtWidgets.QMainWindow()
            self.uic2 = Ui_Dialog()
            self.uic2.setupUi(self.suatuoi)
    ...
    def ok(self):
        global arduino,student,student_name,drink,type,types,myconn,dangxuly,vichat,luongnuoc
        dt = datetime.now()
        value_s=self.uic2.scroll_custem.value()    #kiem tra luong duong qua thanh cuon
        print('day Name:', dt.strftime('%A'))
        if self.uic2.radio_kd.isChecked():         #kiem tra loai nuoc uong 0 la sua
            type=types[0]
        elif self.uic2.radio_id.isChecked():
            type = types[1]                        #1 la sua dau
            if(value_s==0):
                value_s = 3                        #do sua dau ma uong khong duong thi rat do
        elif self.uic2.radio_nd.isChecked():
            type = types[2]                        #2 la nuoc trai cay
            if (value_s == 0):
                value_s = 8                        #do chon khong duong thi rat do
        print(dt.strftime('%A') +'  ')
        print(student_name+' da chon ' +drink+' '+type + ' '+ str(value_s))
        print('co them vi chat loai so ' +str(vichat))
        if vichat!=0:
            data_gui=luongnuoc*100+10+value_s
        else:
            data_gui=luongnuoc*100+value_s
        print(data_gui)
        ...
        print(data_write)
        # vidu : "S18019$"
        # trong do S la sua, 180 la luong nuoc, 1 la vi chat, 9 la ti le duong va $ la ket thuc"
        try:
            arduino.write(bytes(data_write, 'utf-8'))    #chuyen ve byte de qua cong COM

            # self.arduino.write(bytes(data_write))
        except Exception as e:
            print(f'Something went wrong: {e}')
        last_status=''
        while True:  #dung de cho phan hoi tu adruino
            global status
            print(status)
        ...
        try:
            ...
            cur.execute(query, values)
            myconn.commit()
        except:           #neu co loi thi roll back
            myconn.rollback()
        dangxuly =0
        self.suatuoi.close()
