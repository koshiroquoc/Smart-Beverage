...
def xulyQR():
    global cur, dangxuly, student, student_name, day, weekdays, gui_canhbao,gui_baocao,dt,vichat
...
class UI(QtWidgets.QMainWindow, main_page.Ui_MainWindow):
    ...
    def suatuoi(self):
        global student_name,drink,drinks,vichat
        if dangxuly==1:
    ...
    def ok(self):
        global arduino,student,student_name,drink,type,types,myconn,dangxuly,vichat
        dt = datetime.now()
        ...
        try:
            ...
            print(f'Something went wrong: {e}')
        last_status=''
        while False: #chuyeenr thnah false dder tesst
            global status
            print(status)
